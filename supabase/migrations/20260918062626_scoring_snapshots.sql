...
comment on table public.scoring_configs is 'Versioned weights and thresholds. Exactly one active per user.';
...
create unique index scoring_configs_one_active_idx on public.scoring_configs (user_id) where is_active;
...
create table public.skill_scores (
  ...
  gap_type public.gap_type not null default 'none',
  gap_priority numeric,
  created_at timestamptz not null default now(),
  updated_at timestamptz
);
comment on table public.skill_scores is 'Per user skill (optionally per target role), append-mostly (D8).';
...
create index skill_scores_user_skill_id_idx on public.skill_scores (user_skill_id);
create index skill_scores_target_role_id_idx on public.skill_scores (target_role_id);
create unique index skill_scores_one_current_idx
  on public.skill_scores (user_id, user_skill_id, coalesce(target_role_id, '00000000-0000-0000-0000-000000000000'::uuid))
  where is_current;
...
create table public.job_matches (
  ...
  constraint_gate public.constraint_gate not null default 'unknown',
  created_at timestamptz not null default now(),
  updated_at timestamptz
);
comment on table public.job_matches is 'Per job: score, hard-constraint gate, per-requirement breakdown (D6, D8).';
...
create unique index job_matches_one_current_idx on public.job_matches (user_id, job_id) where is_current;
...
comment on table public.readiness_snapshots is 'Per target role: readiness score and breakdown (D8).';
...
create index readiness_snapshots_target_role_id_idx on public.readiness_snapshots (target_role_id);
create unique index readiness_snapshots_one_current_idx
  on public.readiness_snapshots (user_id, target_role_id)
  where is_current;
