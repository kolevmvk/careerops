...
create function public.enforce_roadmap_evidence_required()
returns trigger
language plpgsql
as $$
begin
  if new.status = 'done' and old.status <> 'done' and new.requires_evidence then
    if not exists (
      select 1
      from public.roadmap_item_evidence rie
      join public.evidence e on e.id = rie.evidence_id
...
create trigger enforce_roadmap_evidence_required
  ...
  for each row execute function public.enforce_roadmap_evidence_required();
