...
insert into public.employments (user_id, organization, title, employment_type, start_date, visibility, disclosure_status, ai_allowed)
values (
...
select is(
  (select visibility::text from public.employments where organization = 'Acme Restricted'),
  'cv_safe',
  'a restricted employment is clamped to cv_safe even when set to portfolio_public'
);

select is(
  (select ai_allowed from public.employments where organization = 'Acme Restricted'),
  false,
  'a restricted employment forces ai_allowed to false'
);

-- A highlight on that employment is clamped too, even if set to portfolio_public.
insert into public.employment_highlights (user_id, employment_id, text, visibility)
select '11111111-1111-1111-1111-111111111111', id, 'Shipped a restricted thing', 'portfolio_public'
from public.employments where organization = 'Acme Restricted';

select is(
  (select visibility::text from public.employment_highlights where text = 'Shipped a restricted thing'),
  'cv_safe',
...
insert into public.employments (user_id, organization, title, employment_type, start_date, visibility, disclosure_status)
values (
...
from public.employments where organization = 'OpenCo';

select is(
  (select visibility::text from public.employment_highlights where text = 'Shipped an open thing'),
  'portfolio_public',
