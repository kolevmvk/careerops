...
insert into public.roadmap_items (user_id, title, horizon, requires_evidence)
values ('11111111-1111-1111-1111-111111111111', 'Ship X', 'd90', true);

select throws_like(
  $$ update public.roadmap_items set status = 'done' where title = 'Ship X' $$,
  '%requires at least one verified evidence row%',
  'a roadmap item cannot become done without any linked evidence (I7)'
);

insert into public.evidence (user_id, type, title, occurred_from)
...
select '11111111-1111-1111-1111-111111111111', (select id from public.roadmap_items where title = 'Ship X'), id
from public.evidence where title = 'Design doc';
...
update public.evidence set verified_at = now() where title = 'Design doc';
update public.roadmap_items set status = 'done' where title = 'Ship X';

select is(
  (select status::text from public.roadmap_items where title = 'Ship X'),
  'done',
