...
from careerops.skill_categories where name = 'Cloud';
...
select '11111111-1111-1111-1111-111111111111', 'IAM', 'iam', 'platform', id,
       (select id from careerops.skills where slug = 'aws')
from careerops.skill_categories where name = 'Cloud';

select is((select count(*)::int from careerops.skills where slug = 'iam'), 1, 'a child of a root skill is allowed');

select throws_like(
  $$ insert into careerops.skills (user_id, name, slug, kind, category_id, parent_id)
     select '11111111-1111-1111-1111-111111111111', 'IAM Policies', 'iam-policies', 'platform',
            (select id from careerops.skill_categories where name = 'Cloud'),
            (select id from careerops.skills where slug = 'iam') $$,
...
select throws_like(
  $$ update careerops.skills set parent_id = (select id from careerops.skills where slug = 'other') where slug = 'aws' $$,
  '%already has children%',
  'a skill with existing children cannot become a child itself'
);

select throws_like(
  $$ update careerops.skills set parent_id = id where slug = 'other' $$,
  '%skills_not_own_parent%',
  'a skill cannot be its own parent'
);

select is(
  (select parent_id from careerops.skills where slug = 'other'),
  null,
