...
select is(
  (
    ...
    where n.nspname = 'careerops' and c.relkind = 'r' and not c.relrowsecurity
  ),
  0,
  'every table in careerops has row level security enabled'
);
  ...
  (
    ...
    where n.nspname = 'careerops' and c.relkind = 'r'
      and not exists (
        ...
        where p.schemaname = 'careerops' and p.tablename = c.relname and p.cmd = 'SELECT'
      )
  ),
  0,
...
select is(
  (
    ...
    where n.nspname = 'careerops' and c.relkind = 'r'
      and not exists (
        select 1 from pg_policies p
        where p.schemaname = 'careerops' and p.tablename = c.relname and p.cmd = 'INSERT'
      )
  ),
  0,
...
select is(
  (select count(*)::int from careerops.skill_categories),
  0,
  'user 2 cannot see user 1''s row'
);

insert into careerops.skill_categories (user_id, name) values ('22222222-2222-2222-2222-222222222222', 'Networking');
select is(
  (select count(*)::int from careerops.skill_categories),
  1,
...
select is(
  (select count(*)::int from careerops.skill_categories where user_id = '11111111-1111-1111-1111-111111111111'),
  0,
